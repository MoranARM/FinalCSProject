class Ghost{
  ...
  setNodes(){
    ...
    console.log("YOU LIARRRRRRRRRRRRRRRRRRRRRRRRRRRRR");
      this.ghostNodes[this.ghostNodes.length] = new Node(13, 11);
    }else{
      if(this.chase){
        if(this.clyd){//true if ghost is orange
          if(dist((this.gpos.x-8)/16, (this.gpos.y-8)/16, (pacman.pos.x-8)/16, (pacman.pos.y-8)/16)>8){
            this.ghostNodes[this.ghostNodes.length] = new Node((pacman.pos.x-8)/16, (pacman.pos.y-8)/16);//go after pacman
          }else{
            this.ghostNodes[this.ghostNodes.length] = new Node(this.scatx, this.scaty);//run away from pacman
          }
          ...
          }this.ghostNodes[this.ghostNodes.length] = (dist((this.gpos.x-8)/16, (this.gpos.y-8)/16, pacArrPos.x, pacArrPos.y)<1) ? new Node((pacman.pos.x-8)/16, (pacman.pos.y-8)/16) : new Node(pacArrPos.x, pacArrPos.y);
          ...
          this.ghostNodes[this.ghostNodes.length] = (dist((this.gpos.x-8)/16, (this.gpos.y-8)/16, closestTile.x, closestTile.y)<1) ? new Node((pacman.pos.x-8)/16, (pacman.pos.y-8)/16) : new Node(closestTile.x, closestTile.y);
        }else{//target pacman, what Blinky uses
          this.ghostNodes[this.ghostNodes.length] = new Node((pacman.pos.x-8)/16, (pacman.pos.y-8)/16);
        }
      }else{
        this.ghostNodes[this.ghostNodes.length] = new Node(this.scatx, this.scaty);//scatter to corner
      }
