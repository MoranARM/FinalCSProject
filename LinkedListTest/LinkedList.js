class LinkedList{
  ...
  getLast(){//returns the last element in the LinkedList
    return this.getSize()>0 ? this.getAt(this.size-1) : null;
  }
  
  getSize(){//not used because it takes O(n) to run while .size is O(1)
    let num = 0;
    let temp = this.head;
    if(temp!=null){
      num++;
  ...
  toString(){
    let output = "head = "+(this.head!=null?this.head.data:null)+" size is "+this.size+", or "+this.getSize()+", list contains:";
